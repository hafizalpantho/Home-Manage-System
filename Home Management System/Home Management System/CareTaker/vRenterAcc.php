...
<title>View Renter Balance</title>
<body >
<center>
...
<form action="" method="post">
  ...
  <?php } ?>
    ...
    <table>
      <tr>
          <td  ><input type="submit" name="view" class="do"   value="View Renter Balance Info"></td>
  ...
  if(isset($_POST['view'])){ ?>
    <table align = "center" border="1" style="color:black; width:80% ; font-size:25px;">
      ...
      <tbody>
        ...
        <?php
        $sql = "select * from caretaker where name = '$sessionUname'";
        ...
        $sql = "select name, rent, elecBill, waterBill, utility, penalty,mealBill,currBalance,totalBill, ownerName from account where ownerName = '$owner';";
        $result = mysqli_query($conn, $sql);
        while ($row = mysqli_fetch_assoc($result)){ ?>
          <tr>
